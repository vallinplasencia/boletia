...
func New() *Server {
	...
	// handlers request
	handlersConf, e := aphand.ConfigFromEnv(projectName)
	we("unable read handlers environment", e)
	handlers := aphand.New(handlersConf, db)
	...
	routerConf, e := aprouter.ConfigFromEnv(projectName)
	we("unable read router environment", e)
	r := aprouter.New(routerConf, eng, handlers)
	r.InitRouters()
